...
$(function() {
	...
	$('#LoginFormSimple').find('tbody').prepend(
		'<tr> <td>言語設定</td> <td><select name="locale"> <option value="ja_JP">日本語</option> <option value="en_US">英語(Beta)</option> </select></td> </tr>'
	);
	// ログイン画面のお知らせの項目にBetterKVCの告知を追加する
	var t1 = $('#login-sec1-msg').prepend(
		'<h2 class="ja">Welcome to BetterKVC!!</h2><p class="ja">ようこそ、BetterKVCへ。この拡張機能を使用するとKVCを自分好みにカスタマイズできます。<br>また、BetterKVCのソースコードはGitHub上で公開しています。</p><br>'
	);
	// 上の文字をBetterKVCとバージョン/ビルド番号に変える。
	$('#login-top-title').text("BetterKVC Ver" + version + " Build" + build);
	// 画像を差し替える
	$('img[src="/campusweb/theme/default/newportal/pub/images/kvc_top.gif"]').attr(
		'src', 'https://i.imgur.com/DPAxllI.png');
	$('img[src="/campusweb/theme/default/newportal/image/icon/kvc_login.png"]').attr(
		'src', 'https://i.imgur.com/HOGOTfX.png').attr('width', '240').attr(
		'height', '48');
		...
		if (response.data) {
			...
			for (var i = 0; i < c.length; i++) {
				...
				var name = c[i].replace("https://hastebin.com/raw/", "").replace(".js",
					"").replace("https://aethernote.net/bkvc/", "").replace(
					"https://paste.myftb.de/raw/", "");
				rel = rel + '<li>' + name + '</li>';
				kd++;
				//alert(run+" == "+name);
				var url = c[i];
				...
				if (run != name && canrun) {
					...
					sleep(i, function() {
						try {
							...
							name = ut.replace("https://aethernote.net/bkvc/", "").replace(
								".js", "");
							rel = rel + '<li>' + name + '</li>';
							run = name;
			...
			sleep(0, function() {
				try {
					...
					run = name;
					xhrs[mt] = new XMLHttpRequest();
					xhrs[mt].open("GET", "https://aethernote.net/bkvc/BetterKVCCore.js",
						true);
					xhrs[mt].onreadystatechange = function() {
						if (xhrs[mt].readyState == 4) {
							var resp = eval("" + xhrs[mt].responseText + "");
			...
			sidePanel("BetterKVCの情報", "https://i.imgur.com/tDVHNN1.png", "<b>プラグイン(1)</b><br><ul>" + rel + "</ul><b>セキュリティロック中</b><br>このページではプラグインが実行されません<br>BetterKVC Ver" + version + " Build" +
				build, 1);
			}else{
			// BetterKVCのAPIを使ってサイドパネルを追加してみる
			sidePanel("BetterKVCの情報", "https://i.imgur.com/tDVHNN1.png", inf+"<b>プラグイン(" +
				kd + ")</b><br><ul>" + rel + "</ul>BetterKVC Ver" + version + " Build" +
				build, 1);
			}
		}
	});
...
function sidePanel(title, icon, body, to) {
	...
	if (to == 2) {
		...
		body = body.split('[[[').join(
			'<div id="webpage-list-title-box"> <div id="webpage-list-title-inner">').split(
			']]]').join('</div> </div>');
		$('#area-m2').prepend(
			'<div class="portlet portlet-l portlet-break" id="main-frame"><div class="portlet-box" id="m2-panels"><div id="m2-panels"><div class="portlet-title clearfix" id="m2-panels"><img id="panel-icon" src="' +
			icon + '"><span id="title">' + title + '</span></div> ' + body +
			'<div id="m2-panels-footer" class="clearfix"><div id="m2-panels-footer-inner">&nbsp;&nbsp;</div></div></div></div></div></div>'
		);
		return;
	}
	// 実際に要素を追加する。
	$('#area-m' + to).prepend(
		'<div class="portlet portlet-s portlet-break" id="bkvc-panels"><div class="portlet-box" id="bkvc-panels"><div class="portlet-title clearfix" id="bkvc-panels"> <img src="' +
		icon + '"><span>' + title + '</span></div>' + body + '</div></div>');
	return;
}
...
function addTopMenu(text, icon, jumpto) {
	...
	$('#logout').before('<a href="' + jumpto +
		'" target="_blank"><ul class="clearfix btn" id="betterkvc-topbtn" style=""><li><img id="logocellbetterkvcimg" alt="" src="' +
		icon + '"></li><li class="txt"><span>' + text + '</span></li></ul></a>');
}
...
function addMenu(id, text, icon, jumpto) {
	// jumptoはそのまま新しいタブで開く
	$('#tab-end-of-dummy').before('<a href="' + jumpto +
		'" target="_blank"><div id="tab-' + id +
		'" class="tabcell" style="width: 69px; height: 61px;"><img src="' + icon +
		'" style="width: 32px; height: 32px; margin-left: 18px;"><p>' + text +
		'</p></div></a>');
}
...
function addMenuE(id, text, icon) {
	// jumptoはそのまま新しいタブで開く
	$('#tab-end-of-dummy').before('<div id="tab-' + id +
		'" class="tabcell" style="width: 69px; height: 61px;"><img src="' + icon +
		'" style="width: 32px; height: 32px; margin-left: 18px;"><p>' + text +
		'</p></div>');
}
...
function addMenuEF(id, text, icon) {
	// jumptoはそのまま新しいタブで開く
	$('#tabtable').prepend('<div id="tab-' + id +
		'" class="tabcell" style="width: 69px; height: 61px;"><img src="' + icon +
		'" style="width: 32px; height: 32px; margin-left: 18px;"><p>' + text +
		'</p></div>');
}
...
function addSubMenu(id, text, icon) {
	$('#tabmenu-ul').prepend('<li class="menu-portlet" style=""><span id=' + id +
		'> <img src="' + icon + '" height="48" width="48"> ' + text +
		' </span></li>')
}
...
function addMenuM() {
	$('#tab-end-of-dummy').before(
		'<a href="https://github.com/t4xd/BetterKVC/wiki" target="_blank"><div id="tab-bkvch" class="tabcell" style="width: 69px; height: 61px;"><img src="https://i.imgur.com/tDVHNN1.png" style="width: 32px; height: 32px; margin-left: 18px;"><p>BetterKVCのヘルプ</p></div></a>'
	);
}

function addTopMenuM() {
	$('#logout').before(
		'<ul class="clearfix btn" id="betterkvc-topbtn" style=""><li><img id="logocellbetterkvcimg" alt="" src="https://i.imgur.com/tDVHNN1.png"></li><li class="txt"><span>BetterKVC b' +
		build + '</span></li></ul>');
}

function sidePanelM() {
	$('#area-m3').prepend(
		'<div class="portlet portlet-s portlet-break" id="m3-panel"><div class="portlet-box" id="m3-panel"><div class="portlet-title clearfix" id="m3-panel"> <img src="https://i.imgur.com/tDVHNN1.png"><span>BetterKVCからのお知らせ</span></div><b>BetterKVCへようこそ。</b><br>BetterKVCの詳しい使い方を見るには、右上のBetterKVCと書かれたボタンを押し、BetterKVCのヘルプを確認してください。</div></div>'
	);
}
